...
def qp_predict_handler(self, summary, sbuf):
    ...
    # we don't use rts here; free this
    #self.rmr_free(sbuf)
    print(pred_msg.encode())
    success = self.rmr_send(pred_msg.encode(), 30002,10000)
    print(success)
    self.rmr_free(sbuf)
...
def cells(ue):
    ...
    """
    db.read_data(meas='liveUE', limit=1000, cellid=False, ueid=ue)
    df = db.data
    cells = []
    if df is not None:
...
def predict(payload):
    ...
    for ueid in ue_list:
        ...
        for cid in cell_list:
            train_model(cid)
            mcid = cid.replace('/', '')
            db.read_data(meas='liveCell', limit=100, cellid=cid, ueid=False)
