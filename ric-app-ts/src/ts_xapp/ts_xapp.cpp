...
void Send_tcp1(unsigned char* payload);
...
void Send_tcp1(unsigned char* payload){
  //if no unsigned char* payload listen from TCP socket from AD xApp
  if(payload !=nullptr){
    int sock=0;
    ...
    if ((sock = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
        std::cout << "[INFO]Socket creation error \n";
    ...
    while (true)
    {
    send(sock, message, strlen(reinterpret_cast<const char*>(message)), 0);
  ...
  cout<<"[INFO] Success send TCP"<<endl;
...
void Send_tcp2(){
    ...
    ssize_t bytes_read = read(new_socket, buffer, 8192);
    std::cout << "Received message: " << buffer << std::endl;
    
    unsigned char* message = reinterpret_cast<unsigned char*>(buffer);
    Send_tcp1(message);
...
extern int main( int argc, char** argv ) {
  ...
  Send_tcp2();
  xfw = std::unique_ptr<Xapp>( new Xapp( port, false ) );
